...
export const universalDraft = (roomId = ""): UniversalDraft => ({
  date: today, duration: "", notes: "", rows: [{ key: uid(), roomId, overallResult: "", cashResult: "", mttResult: "" }],
});

export function normalizeState(raw: AppState): AppState {
  const previousVersion=Number(raw.version||1),baseRooms=raw.rooms.map((room)=>String(room.mode)==="universal"?{...room,mode:"both" as const}:room),rooms=baseRooms.map((room)=>previousVersion<5?{...room,currentBalance:room.currentBalance+raw.rewards.filter((reward)=>reward.roomId===room.id).reduce((sum,reward)=>sum+reward.amount,0)}:room);
  ...
  return { version: 6, profileName: "Denys", rooms,
    ...
    cashSessions: [
      ...
      { id: uid(), date: "2026-08-10", duration: 6.5, notes: "Mixed Sunday grind", entries: [
        { roomId: "stars", overallResult: 300, cashResult: 85, mttResult: 215 },
        { roomId: "gg", overallResult: -120, cashResult: -40, mttResult: -80 },
      ] },
    ], rewards: [
