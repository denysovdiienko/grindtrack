...
export const universalDraft = (roomId = ""): UniversalDraft => ({
  date: today, duration: "", notes: "", rows: [{ key: uid(), roomId, cashResult: "", tournamentBuyIns: "", tournamentCashes: "", sngBuyIns: "", sngCashes: "" }],
});
...
export function normalizeState(raw: AppState): AppState {
  ...
  const firstUniversalRoom = raw.rooms.find((room) => room.kind === "room" && (room.mode === "universal" || room.mode === "both"))?.id || raw.rooms.find((room)=>room.kind==="room")?.id || "";
  const drafts = raw.drafts as Partial<AppState["drafts"]> | undefined;
  return { ...raw, version: 3, universalDays: raw.universalDays || [], drafts: {
    tournament: drafts?.tournament || tournamentDraft(firstTournamentRoom),
    cash: drafts?.cash || cashDraft(firstCashRoom),
    universal: drafts?.universal || universalDraft(firstUniversalRoom),
  } } as AppState;
}
...
export const emptyState = (name: string): AppState => {
  ...
  return { version: 3, profileName: name, rooms, tournamentDays: [], cashSessions: [], universalDays: [], rewards: [], drafts: {
...
export function demoState(): AppState {
  ...
  return { version: 3, profileName: "Denys", rooms,
    ...
    cashSessions: [
      ...
      { id: uid(), date: "2026-08-10", duration: 6.5, notes: "Mixed Sunday grind", entries: [
        { roomId: "stars", cashResult: 85, tournamentBuyIns: 310, tournamentCashes: 540, sngBuyIns: 60, sngCashes: 45 },
        { roomId: "gg", cashResult: -40, tournamentBuyIns: 220, tournamentCashes: 90, sngBuyIns: 80, sngCashes: 130 },
      ] },
    ], rewards: [
